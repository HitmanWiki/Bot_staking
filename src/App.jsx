...
const metadata = {
  name: 'BOT Staking',
  description: 'Stake BOT, Earn rewards',
  url: '',
...
// SVG bg pattern
const backgroundPattern = '/bg.png';

const logoUrl = 'Logo.png';
const gifUrl = 'Bot.gif';
      ...
      {projectedReturns && (
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
            ...
            <span style={{ fontFamily: '"Burger Free", sans-serif;' }}>{projectedReturns.principal.toFixed(2)} BOT</span>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
            <span style={{ color: '#e5008e' }}>Rewards ({projectedReturns.days} days):</span>
            <span style={{ fontFamily: '"Burger Free", sans-serif;', color: '#ab1567' }}>
              +{projectedReturns.reward.toFixed(2)} BOT
            </span>
          </div>
          ...
          <div style={{
            ...
            <span style={{ fontFamily: '"Burger Free", sans-serif;', fontWeight: 'bold', fontSize: '1.25rem' }}>
              {projectedReturns.total.toFixed(2)} BOT
            </span>
          </div>
        </div>
      )}
...
const AppContent = () => {
  ...
  const handleStake = async (poolId) => {
    ...
    if (isNaN(amountNum) || amountNum <= 0) {
      showCustomModal('Bot! Please enter a valid amount.', 'error');
      return;
    }
