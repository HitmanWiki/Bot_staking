...
/**
 * @title AdvancedBOTStaking
 * @dev A staking contract for BOT tokens with multiple, owner-configurable pools.
 * The contract owner is responsible for depositing reward tokens.
 * It includes an emergency unstake feature with a penalty.
 */
contract AdvancedBOTStaking is Ownable, ReentrancyGuard {
    ...
    // Assuming the staked token and reward token are the same (BOT).
    ...
    /**
     * @dev Constructor for the AdvancedBOTStaking contract.
     * Initializes the contract with the BOT token address and sets up the initial staking pools.
     * @param _BOTTokenAddress The address of the BOT ERC20 token.
     * @param owner The address that will own the contract.
     */
    constructor(IERC20 _BOTTokenAddress, address owner) Ownable(owner) {
        BOTToken = _BOTTokenAddress;
    ...
    /**
     * @dev Allows the owner to deposit BOT tokens into the contract as rewards.
     * @param _amount The amount of BOT tokens to deposit.
     */
    function depositRewards(uint256 _amount) external onlyOwner nonReentrant {
        require(_amount > 0, "Amount must be greater than 0");
        BOTToken.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    /**
     * @dev Allows a user to stake BOT tokens for a specific duration.
     * Condition: Rewards must be available in the contract.
     * @param _amount The amount of BOT tokens to stake.
    ...
    function stake(
        ...
        require(BOTToken.balanceOf(address(this)) > 0, "Rewards missing");
    ...
    /**
     * @dev Allows a user to unstake their BOT tokens after the lock-up period has expired.
    ...
    function unstake(uint256 _lockDuration) external nonReentrant {
        ...
        BOTToken.safeTransfer(msg.sender, stakedAmount);
    ...
    function emergencyUnstake(uint256 _lockDuration) external nonReentrant {
        ...
        BOTToken.safeTransfer(msg.sender, amountToReturn);
    ...
    function claimRewards() external nonReentrant {
        ...
        uint256 contractBalance = BOTToken.balanceOf(address(this));
        ...
        BOTToken.safeTransfer(msg.sender, totalRewards);
     ...
     * @return The total amount of rewards in BOT tokens.
     */
